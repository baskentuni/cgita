function metadata = dicominfo(filename, do_fields_table, varargin)
...
%   $Revision: 1.1.6.4 $  $Date: 2010/04/15 15:18:38 $


% Parse input arguments.
if (nargin < 1)
    eid = 'Images:dicominfo:tooFewInputs';
    error(eid, 'DICOMINFO requires at least one argument.')
...
try
    ...
    attrs = dicomparse(fileDetails.name, ...
                   ...
                   vec1 = sum(do_fields_table, 2);
                   attrs2 = attrs(1);
                   for idx2 = 1:length(attrs)
                       if length(find(vec1 == attrs(idx2).Group+attrs(idx2).Element))>0
                           attrs2(end+1) = attrs(idx2);
                       end
                   end

    % Process the raw attributes.
    [metadata,attrNames] = processMetadata(attrs2, true);
    ...
    metadata = processCurveData(metadata, attrs2, attrNames);
...
function [metadata,attrNames] = processMetadata(attrs, isTopLevel)
    ...
    for k = 1:2:length(varargin)
        ...
        case 'dictionary'
            ...
            if (k == length(varargin))
                ...
                error(eid, 'No data dictionary specified.');
            else
...
if (isempty(details))
    ...
    if (isempty(file.Filename))
        ...
        error(eid, 'File "%s" not found.', filename);
    end
    ...
    if ~isfield(in,dvrName)
        ...
        switch dataType
            ...
            case 4
                expDataType = 'int32';
            otherwise
                warning('Images:dicominfo:unknownDataType', ...
                    'Unknown data value representation for curve data: %d.', ...
                    dataType);
        end
